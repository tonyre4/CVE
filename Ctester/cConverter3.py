...
def compile(option):
    ...
    for x in files2:
        ...
        subprocess.check_output(['rm files/building.py ; mv output.py files/building.py || true'],
                                shell=True)
    ##Parte que quita sys
    #print subprocess.check_output(['grep "sys" files/building.py || true'] , shell = True)
...
def clean(option):
    ...
    if option == 3:
        ...
        subprocess.check_output(['ls | grep -v ".png" | grep -v "cConver" | grep -v "final" | xargs rm || true'], shell=True)
    else:
...
ext_modules = [Extension('test', ['final.py'], )]
